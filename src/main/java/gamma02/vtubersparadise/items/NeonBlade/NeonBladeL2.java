...
public class NeonBladeL2 extends NeonBladeL1
{

    public NeonBladeL2(IItemTier tier, int attackDamageIn, float attackSpeedIn, Properties builderIn)
    {
        ...
        if (entityLiving instanceof PlayerEntity) {
            ...
            if (i >= 10) {
                ...
                if (j <= 0 || playerentity.isWet()) {
                    if (!worldIn.isRemote) {
                        if (j == 0) {
                            NeonSwordEntity tridententity = new NeonSwordEntity(worldIn, playerentity, stack);
